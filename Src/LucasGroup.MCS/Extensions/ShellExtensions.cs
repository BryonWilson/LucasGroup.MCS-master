...
namespace LucasGroup.MCS.Extensions
{
    public static class ShellExtensions
    {
        ...
        public static void Shell(this IApplicationBuilder app, string cmd, string dir)
        {
            try
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    process = new Process
                    {
                        ...
                        StartInfo = new ProcessStartInfo("cmd.exe", "/C " + cmd) { WorkingDirectory = dir, UseShellExecute = false }
                    };
                }
